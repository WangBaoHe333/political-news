...
def _render_news_card(item, keyword: Optional[str] = None) -> str:
    ...
    return (
        ...
        f"<div class='news-meta'><span>{published}</span><span>导读节选</span></div>"
        f"{_render_source_signature(item)}"
        f"<h4><a href='/news/{item.id}'>{title}</a></h4>"
        f"<p>{excerpt}</p>"
        "<div class='card-actions'>"
        f"<a class='inline-link' href='/news/{item.id}'>站内查看</a>"
        f"<a class='inline-link' href='{link}' target='_blank' rel='noreferrer'>前往原文</a>"
        "</div>"
        "</article>"
    )
...
def _render_quality_panel() -> str:
    ...
    return (
        ...
        "<div class='notice compact'><strong>展示方式：</strong>站内仅提供公开来源导读与有限节选，不做 AI 改写，也不镜像完整原文，并始终保留原文链接。</div>"
...
async def news_detail_page(news_id: int):
    ...
    main_html = (
        ...
        "</div>"
        "<div class='actions'>"
        f"<a class='ghost-link' href='{escape(item.link)}' target='_blank' rel='noreferrer'>前往原站全文</a>"
        f"<a class='ghost-link' href='{_build_href(f'/year/{item.year}', source=item.source)}'>查看同年同来源</a>"
        "</div>"
        "<div class='notice'><strong>站内说明：</strong>本站基于公开来源提供导读与有限节选，便于快速阅读；完整表述与上下文请以原站全文为准。</div>"
        f"<div class='notice'><strong>导读：</strong>{escape(item.summary or '暂无导读')}</div>"
