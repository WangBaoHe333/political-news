...
一个面向时政学习与日常查阅的资料库，提供近两年时政数据同步、数据库检索、独立页面导航、按月归档、今日/昨日查看、年份切换与关键词搜索等功能。

## ✨ 功能特性

- **可信来源抓取**：当前只纳入中国政府网、人民网、新华网、中国新闻网等白名单来源
- **数据库驱动界面**：页面直接从数据库取数，不再依赖临时拼装展示
- **真实性说明**：站内展示为原文摘录，不做 AI 改写，始终保留原文链接与来源说明
- **独立页面导航**：
  - 今日时政
  - 昨日时政
  - 分类专题
  ...
  - 按月归档
  - 同步状态
- **按月归档与分页**：归档默认折叠，页面避免无限变长
- **后台同步**：支持启动时自动补齐近两年数据，也支持手动触发同步
- **RESTful API**：保留 JSON 接口，便于后续扩展
- **响应式 Web 界面**：适配桌面与移动端
...
HTTP_VERIFY_TLS=1                     # 抓取时默认校验证书，生产强烈建议保持开启
...
### 同步管理API
- `GET /sync` - 手动触发同步（支持 `X-Sync-Token` 请求头）
...
### 3. 推荐：使用 systemd 托管（避免重启后服务丢失）

```bash
cd /opt/political-news

# 首次部署建议先准备 Python 虚拟环境
python3 -m venv .venv
...
sudo INSTALL_DIR=/opt/political-news RUN_USER=root RUN_GROUP=root ./scripts/install-systemd.sh
...
说明：
- 安装脚本会优先把 `.env.production`（或 `.env`）复制为 `.env.runtime`，供 systemd 读取
- 服务会开机自启，异常自动重启，日志写入 `/opt/political-news/logs/app.log`
- 如需修改运行参数，编辑 `/opt/political-news/.env.runtime` 后执行 `sudo systemctl restart political-news.service`
